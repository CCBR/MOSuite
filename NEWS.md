...
- Update S7 class, generic, and method documentation to use roxygen2 v8.0.0 S7 support: add `@property` tags for `multiOmicDataSet` class properties, replace `@method` workarounds with `@exportS7Method`, and remove redundant `@rdname`/`@name` tags from generic definitions. (#207, @copilot)
