...
- change_colname:
  ...
  Character vector of full logFC column names, one per contrast (e.g.
  `c("B-A_logFC", "C-A_logFC")`). Defaults to `NULL`, which auto-detects
  all columns ending in `_logFC`.

- signif_colname:

  Character vector of full significance column names, one per contrast
  (e.g. `c("B-A_adjpval", "C-A_adjpval")`). Defaults to `NULL`, which
...
- label_font_size:
  ...
  Set the font size of the labels. Default: 7

- draw_connectors:

  If `TRUE`, draw connector lines from labels to their points and spread
...
- title_font_size:
  ...
  Size of the plot title. Default: 24

- use_custom_lab:

  If TRUE, uses custom labels for the plot axes, set by
  `change_sig_name` and `change_lfc_name`.

- color_of_signif_threshold_line:

  Color of the significance threshold line. Default: "black"

- color_of_non_significant_features:

  Color of the non-significant features. Default: "grey30"

- color_of_logfold_change_threshold_line:

  Color of the log fold change threshold line. Default: "forestgreen"
...
- axis_lab_size:
  ...
  Size of the axis labels. Default: 24

- axis_tick_lab_size:

  Size of the axis tick labels. Default: 16
