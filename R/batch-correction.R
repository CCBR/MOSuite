# Batch Correction [CCBR] (4d5e38c8-27e7-4bce-a739-2120eb8c71fb): v216
batch_correct <- function(NormalizedCounts, metadata_ccbr_bulk_training) {
  ## --------- ##
  ## Libraries ##
  ## --------- ##
  ...
  ## Functions ##
  ## --------- ##
  ### PH: START Color Picking Function - from Filtering + Normalization Template
  ...
  ### PH: END Color Picking Function - from Filtering + Normalization Template


  ### PH: START Correlation Heatmap Function - from Filtering + Normalization Template
  ...
  if (skip_batch_correction == FALSE) {
    if (length(unique(Batch)) <= 1) {
      ...
      combat_edata <- ComBat(as.matrix(df.filt),
  ...
  ## PCA Plot:
  ########################
  ## same function as used from Filter Template except input is Batch Corrected Counts table instead of Raw Data.
  ## major difference is that rawData had to be log transformed. This Data should be similar to the Normalize Data Template
  ...
  ### PH: END PCA Plot Input is Counts table and should match function from Filter and Normalizaiton

  ### PH: START Use rename_samples previously generated for Filter Function.
  ### Should this function be part of all plotting functions or a step in processing the input table before plotting?
  ...
  ########################
  ## same function as used from Filter Template + Normalization Template.
  ## only change I think we need is to recognize the type of Counts input to label X axis ( is this normalized, or Raw Counts, or Batch Corrected)
