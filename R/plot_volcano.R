...
plot_volcano_summary <- function(DEGAnalysis) {
  ...
  for(contrast in contrasts){
    ...
    ### PH: START Buid table for Volcano plot
    ...
    ### PH: END Buid table for Volcano plot
...
plot_volcano_enhanced <- function(DEGAnalysis) {
  ...
  for(i in 1:length(lfc.col)){
    ...
    ## This is unique to this template and could be usefull as a generic tool to create rankes for GSEA. Recomend extracting this function
    ...
    ## Createing plot that can be converted to plotly interactive plot (no labels):
    ## PH: make this feature an option not defult
