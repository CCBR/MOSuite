...
log_transform_counts <- function(
  ...
  if (any(as.matrix(counts_dat[, sample_colnames, drop = FALSE]) + pseudocount <= 0, na.rm = TRUE)) {
    stop("log_transform_counts requires all count values plus pseudocount to be greater than 0")
  }
  counts_dat |>
...
resolve_log_transform_base <- function(base) {
  if (is.character(base) && length(base) == 1) {
    ...
    stop("base must be a single numeric value or one of 'e', 'ln', or 'natural'")
  }
