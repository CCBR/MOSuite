# Package index

## Main analysis functions

See
[`vignette('intro')`](https://ccbr.github.io/MOSuite/articles/intro.md)
for recommended usage

- [`batch_correct_counts()`](https://ccbr.github.io/MOSuite/reference/batch_correct_counts.md)
  : Perform batch correction
- [`clean_raw_counts()`](https://ccbr.github.io/MOSuite/reference/clean_raw_counts.md)
  : Clean Raw Counts
- [`diff_counts()`](https://ccbr.github.io/MOSuite/reference/diff_counts.md)
  : Differential expression analysis
- [`filter_counts()`](https://ccbr.github.io/MOSuite/reference/filter_counts.md)
  : Filter low counts
- [`filter_diff()`](https://ccbr.github.io/MOSuite/reference/filter_diff.md)
  : Filter features from differential analysis based on statistical
  significance
- [`normalize_counts()`](https://ccbr.github.io/MOSuite/reference/normalize_counts.md)
  : Normalize counts

## multiOmicDataSet class

### multiOmicDataSet constructors, readers, & writers

- [`create_multiOmicDataSet_from_dataframes()`](https://ccbr.github.io/MOSuite/reference/create_multiOmicDataSet_from_dataframes.md)
  : Construct a multiOmicDataSet object from data frames

- [`create_multiOmicDataSet_from_files()`](https://ccbr.github.io/MOSuite/reference/create_multiOmicDataSet_from_files.md)
  : Construct a multiOmicDataSet object from text files (e.g. TSV, CSV).

- [`multiOmicDataSet()`](https://ccbr.github.io/MOSuite/reference/multiOmicDataSet.md)
  :

  `multiOmicDataSet` class

- [`read_multiOmicDataSet()`](https://ccbr.github.io/MOSuite/reference/read_multiOmicDataSet.md)
  : Read a multiOmicDataSet from RDS

- [`read_multiOmicDataSet_properties()`](https://ccbr.github.io/MOSuite/reference/read_multiOmicDataSet_properties.md)
  : Read multiOmicDataSet properties from individual files.

- [`write_multiOmicDataSet()`](https://ccbr.github.io/MOSuite/reference/write_multiOmicDataSet.md)
  : Write a multiOmicDataSet to RDS

- [`write_multiOmicDataSet_properties()`](https://ccbr.github.io/MOSuite/reference/write_multiOmicDataSet_properties.md)
  : Write multiOmicDataSet properties to individual files.

### multiOmicDataSet methods

All other functions that accept a `multiOmicDataSet` as the first
argument.

- [`batch_correct_counts()`](https://ccbr.github.io/MOSuite/reference/batch_correct_counts.md)
  : Perform batch correction
- [`calc_cpm()`](https://ccbr.github.io/MOSuite/reference/calc_cpm.md) :
  Calculate counts-per-million (CPM) on raw counts in a multiOmicDataSet
- [`clean_raw_counts()`](https://ccbr.github.io/MOSuite/reference/clean_raw_counts.md)
  : Clean Raw Counts
- [`diff_counts()`](https://ccbr.github.io/MOSuite/reference/diff_counts.md)
  : Differential expression analysis
- [`extract_counts()`](https://ccbr.github.io/MOSuite/reference/extract_counts.md)
  : Extract count data
- [`filter_counts()`](https://ccbr.github.io/MOSuite/reference/filter_counts.md)
  : Filter low counts
- [`filter_diff()`](https://ccbr.github.io/MOSuite/reference/filter_diff.md)
  : Filter features from differential analysis based on statistical
  significance
- [`normalize_counts()`](https://ccbr.github.io/MOSuite/reference/normalize_counts.md)
  : Normalize counts
- [`plot_corr_heatmap()`](https://ccbr.github.io/MOSuite/reference/plot_corr_heatmap.md)
  : Plot correlation heatmap
- [`plot_expr_heatmap()`](https://ccbr.github.io/MOSuite/reference/plot_expr_heatmap.md)
  : Plot expression heatmap
- [`plot_histogram()`](https://ccbr.github.io/MOSuite/reference/plot_histogram.md)
  : Plot histogram
- [`plot_pca()`](https://ccbr.github.io/MOSuite/reference/plot_pca.md) :
  Perform and plot a Principal Components Analysis
- [`plot_pca_2d()`](https://ccbr.github.io/MOSuite/reference/plot_pca_2d.md)
  : Perform and plot a 2D Principal Components Analysis
- [`plot_pca_3d()`](https://ccbr.github.io/MOSuite/reference/plot_pca_3d.md)
  : Perform and plot a 3D Principal Components Analysis
- [`plot_read_depth()`](https://ccbr.github.io/MOSuite/reference/plot_read_depth.md)
  : Plot read depth as a bar plot
- [`plot_venn_diagram()`](https://ccbr.github.io/MOSuite/reference/plot_venn_diagram.md)
  : Plot a venn diagram, UpSet plot, or table of intersections
- [`plot_volcano_enhanced()`](https://ccbr.github.io/MOSuite/reference/plot_volcano_enhanced.md)
  : Enhanced Volcano Plot
- [`plot_volcano_summary()`](https://ccbr.github.io/MOSuite/reference/plot_volcano_summary.md)
  : Volcano Plot - Summary
- [`set_color_pal()`](https://ccbr.github.io/MOSuite/reference/set_color_pal.md)
  : Set color palette for a single group/column
- [`set_default_colors()`](https://ccbr.github.io/MOSuite/reference/set_default_colors.md)
  : Set default colors on a multiOmicDataSet

## Visualization

- [`plot_corr_heatmap()`](https://ccbr.github.io/MOSuite/reference/plot_corr_heatmap.md)
  : Plot correlation heatmap
- [`plot_expr_heatmap()`](https://ccbr.github.io/MOSuite/reference/plot_expr_heatmap.md)
  : Plot expression heatmap
- [`plot_histogram()`](https://ccbr.github.io/MOSuite/reference/plot_histogram.md)
  : Plot histogram
- [`plot_pca()`](https://ccbr.github.io/MOSuite/reference/plot_pca.md) :
  Perform and plot a Principal Components Analysis
- [`plot_read_depth()`](https://ccbr.github.io/MOSuite/reference/plot_read_depth.md)
  : Plot read depth as a bar plot
- [`plot_venn_diagram()`](https://ccbr.github.io/MOSuite/reference/plot_venn_diagram.md)
  : Plot a venn diagram, UpSet plot, or table of intersections
- [`plot_volcano_enhanced()`](https://ccbr.github.io/MOSuite/reference/plot_volcano_enhanced.md)
  : Enhanced Volcano Plot
- [`plot_volcano_summary()`](https://ccbr.github.io/MOSuite/reference/plot_volcano_summary.md)
  : Volcano Plot - Summary
- [`print_or_save_plot()`](https://ccbr.github.io/MOSuite/reference/print_or_save_plot.md)
  : Print and/or save a ggplot
- [`render_report()`](https://ccbr.github.io/MOSuite/reference/render_report.md)
  : Render the template report

### Color palette

- [`display_colors()`](https://ccbr.github.io/MOSuite/reference/display_colors.md)
  : Display colors for a multiOmicDataSet object
- [`get_colors_lst()`](https://ccbr.github.io/MOSuite/reference/get_colors_lst.md)
  : Create named list of default colors for plotting
- [`get_colors_vctr()`](https://ccbr.github.io/MOSuite/reference/get_colors_vctr.md)
  : Get vector of colors for observations in one column of a data frame
- [`select_mosuite_colors()`](https://ccbr.github.io/MOSuite/reference/select_mosuite_colors.md)
  : Select colors from MOSuite's default palette
- [`set_color_pal()`](https://ccbr.github.io/MOSuite/reference/set_color_pal.md)
  : Set color palette for a single group/column
- [`set_default_colors()`](https://ccbr.github.io/MOSuite/reference/set_default_colors.md)
  : Set default colors on a multiOmicDataSet
- [`display_palette()`](https://ccbr.github.io/MOSuite/reference/display_palette.md)
  : Display the mosuite color palette
- [`mosuite_palette`](https://ccbr.github.io/MOSuite/reference/mosuite_palette.md)
  : Default MOSuite color palette

### Low-level plot helpers

These functions are called by the main visualization functions.

- [`plot_corr_heatmap,data.frame-method`](https://ccbr.github.io/MOSuite/reference/plot_corr_heatmap-data.frame.md)
  [`plot_corr_heatmap.data.frame`](https://ccbr.github.io/MOSuite/reference/plot_corr_heatmap-data.frame.md)
  : Plot correlation heatmap for counts dataframe

- [`plot_corr_heatmap,MOObject::multiOmicDataSet-method`](https://ccbr.github.io/MOSuite/reference/plot_corr_heatmap-multiOmicDataSet.md)
  [`plot_corr_heatmap.multiOmicDataSet`](https://ccbr.github.io/MOSuite/reference/plot_corr_heatmap-multiOmicDataSet.md)
  : Plot correlation heatmap for multiOmicDataSet

- [`plot_histogram,data.frame-method`](https://ccbr.github.io/MOSuite/reference/plot_histogram.data.frame.md)
  [`plot_histogram.data.frame`](https://ccbr.github.io/MOSuite/reference/plot_histogram.data.frame.md)
  : Plot histogram for counts dataframe

- [`plot_histogram,MOObject::multiOmicDataSet-method`](https://ccbr.github.io/MOSuite/reference/plot_histogram.multiOmicDataSet.md)
  [`plot_histogram.multiOmicDataSet`](https://ccbr.github.io/MOSuite/reference/plot_histogram.multiOmicDataSet.md)
  : Plot histogram for multiOmicDataSet

- [`plot_pca,data.frame-method`](https://ccbr.github.io/MOSuite/reference/plot_pca.data.frame.md)
  [`plot_pca.data.frame`](https://ccbr.github.io/MOSuite/reference/plot_pca.data.frame.md)
  : Plot 2D or 3D PCA for counts dataframe

- [`plot_pca,MOObject::multiOmicDataSet-method`](https://ccbr.github.io/MOSuite/reference/plot_pca.multiOmicDataSet.md)
  [`plot_pca.multiOmicDataSet`](https://ccbr.github.io/MOSuite/reference/plot_pca.multiOmicDataSet.md)
  : Plot 2D or 3D PCA for multiOmicDataset

- [`plot_pca_2d()`](https://ccbr.github.io/MOSuite/reference/plot_pca_2d.md)
  : Perform and plot a 2D Principal Components Analysis

- [`plot_pca_3d()`](https://ccbr.github.io/MOSuite/reference/plot_pca_3d.md)
  : Perform and plot a 3D Principal Components Analysis

- [`plot_read_depth,data.frame-method`](https://ccbr.github.io/MOSuite/reference/plot_read_depth.data.frame.md)
  [`plot_read_depth.data.frame`](https://ccbr.github.io/MOSuite/reference/plot_read_depth.data.frame.md)
  :

  Plot read depth for `data.frame`

- [`plot_read_depth,MOObject::multiOmicDataSet-method`](https://ccbr.github.io/MOSuite/reference/plot_read_depth.multiOmicDataSet.md)
  [`plot_read_depth.multiOmicDataSet`](https://ccbr.github.io/MOSuite/reference/plot_read_depth.multiOmicDataSet.md)
  : Plot read depth for multiOmicDataSet

## Misc. helpers & utilities

- [`calc_cpm()`](https://ccbr.github.io/MOSuite/reference/calc_cpm.md) :
  Calculate counts-per-million (CPM) on raw counts in a multiOmicDataSet
- [`calc_pca()`](https://ccbr.github.io/MOSuite/reference/calc_pca.md) :
  Perform principal components analysis
- [`bind_dfs_long()`](https://ccbr.github.io/MOSuite/reference/bind_dfs_long.md)
  : Bind dataframes in named list to long dataframe
- [`join_dfs_wide()`](https://ccbr.github.io/MOSuite/reference/join_dfs_wide.md)
  : Join dataframes in named list to wide dataframe

## Datasets

- [`gene_counts`](https://ccbr.github.io/MOSuite/reference/gene_counts.md)
  : RSEM expected gene counts

- [`mosuite_palette`](https://ccbr.github.io/MOSuite/reference/mosuite_palette.md)
  : Default MOSuite color palette

- [`nidap_batch_corrected_counts`](https://ccbr.github.io/MOSuite/reference/nidap_batch_corrected_counts.md)
  : Batch-corrected counts for the NIDAP test dataset.

- [`nidap_batch_corrected_counts_2`](https://ccbr.github.io/MOSuite/reference/nidap_batch_corrected_counts_2.md)
  :

  Batch-corrected counts for the NIDAP test dataset. The result of
  running
  [`batch_correct_counts()`](https://ccbr.github.io/MOSuite/reference/batch_correct_counts.md)
  on `nidap_norm_counts`.

- [`nidap_clean_raw_counts`](https://ccbr.github.io/MOSuite/reference/nidap_clean_raw_counts.md)
  :

  Clean raw counts for the NIDAP test dataset. The result of running
  [`clean_raw_counts()`](https://ccbr.github.io/MOSuite/reference/clean_raw_counts.md)
  on `nidap_raw_counts`.

- [`nidap_deg_analysis`](https://ccbr.github.io/MOSuite/reference/nidap_deg_analysis.md)
  : Differential gene expression analysis for the NIDAP test dataset.

- [`nidap_deg_analysis_2`](https://ccbr.github.io/MOSuite/reference/nidap_deg_analysis_2.md)
  :

  Differential gene expression analysis for the NIDAP test dataset. The
  result of running
  [`diff_counts()`](https://ccbr.github.io/MOSuite/reference/diff_counts.md)
  on `nidap_filtered_counts`.

- [`nidap_deg_gene_list`](https://ccbr.github.io/MOSuite/reference/nidap_deg_gene_list.md)
  :

  List of differentially expressed genes from the NIDAP test dataset
  using default parameters with
  [`filter_diff()`](https://ccbr.github.io/MOSuite/reference/filter_diff.md).

- [`nidap_filtered_counts`](https://ccbr.github.io/MOSuite/reference/nidap_filtered_counts.md)
  :

  Filtered counts for the NIDAP test dataset. The result of running
  [`filter_counts()`](https://ccbr.github.io/MOSuite/reference/filter_counts.md)
  on `nidap_clean_raw_counts`.

- [`nidap_norm_counts`](https://ccbr.github.io/MOSuite/reference/nidap_norm_counts.md)
  :

  Normalized counts for the NIDAP test dataset. The result of running
  [`normalize_counts()`](https://ccbr.github.io/MOSuite/reference/normalize_counts.md)
  on `nidap_filtered_counts`.

- [`nidap_raw_counts`](https://ccbr.github.io/MOSuite/reference/nidap_raw_counts.md)
  :

  Raw counts for the NIDAP test dataset Pairs with
  `nidap_sample_metadata`.

- [`nidap_sample_metadata`](https://ccbr.github.io/MOSuite/reference/nidap_sample_metadata.md)
  : Sample metadata for the NIDAP test dataset

- [`nidap_venn_diagram_dat`](https://ccbr.github.io/MOSuite/reference/nidap_venn_diagram_dat.md)
  :

  Output data from venn diagram. The result of running
  [`plot_venn_diagram()`](https://ccbr.github.io/MOSuite/reference/plot_venn_diagram.md)
  on `nidap_volcano_summary_dat`

- [`nidap_volcano_summary_dat`](https://ccbr.github.io/MOSuite/reference/nidap_volcano_summary_dat.md)
  : Summarized differential expression analysis for input to venn
  diagram

## Options

- [`options`](https://ccbr.github.io/MOSuite/reference/options.md) :
  MOSuite Options

## Re-exports

- [`reexports`](https://ccbr.github.io/MOSuite/reference/reexports.md)
  [`:=`](https://ccbr.github.io/MOSuite/reference/reexports.md)
  [`!!`](https://ccbr.github.io/MOSuite/reference/reexports.md)
  [`.data`](https://ccbr.github.io/MOSuite/reference/reexports.md)
  [`%||%`](https://ccbr.github.io/MOSuite/reference/reexports.md) :
  Objects exported from other packages
