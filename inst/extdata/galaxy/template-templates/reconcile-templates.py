        ...
        with open(code_template_file, 'r') as infile:
            ...
            for arg_type in ('columns', 'inputDatasets', 'parameters'):
                for param in code_template.get(arg_type, []):
                    ...
                    params_lst = next((m for k, m in mapping.get('template_mappings', {}).items() if k == template_name), {}).get('parameter_mappings', [])
                    new_param  = next((p.get(param_name) for p in params_lst if param_name in p), None)
                    if new_param:
                        param['key'] = new_param
                        new_template[arg_type].append(param)
                    else:
                        new_template[arg_type].append(param)
            with open(pathlib.Path(f'inst/extdata/galaxy/template-templates/{template_name}'), 'w') as outfile:
