# Authors and Citation

## Authors

- **[Kelly Sovacool](https://github.com/kelly-sovacool)**. Author,
  maintainer. [](https://orcid.org/0000-0003-3283-829X)

- **[Philip Homan](https://github.com/phoman14)**. Author.

- **[Vishal Koparde](https://github.com/kopardev)**. Author.
  [](https://orcid.org/0000-0001-8978-8495)

- **Samantha Chill**. Author. [](https://orcid.org/0000-0002-8734-9875)

- **T. Joshua Meyer**. Contributor.

- **[CCR Collaborative Bioinformatics
  Resource](https://github.com/CCBR)**. Copyright holder.

## Citation

Source:
[`inst/CITATION`](https://github.com/CCBR/MOSuite/blob/v0.3.1/inst/CITATION)

Sovacool K, Homan P, Koparde V, Chill S (2025). *MOSuite: R package for
differential multi-omics analysis*.
[doi:10.5281/zenodo.16371580](https://doi.org/10.5281/zenodo.16371580),
<https://ccbr.github.io/MOSuite/>.

    @Manual{sovacool_mosuite_2025,
      title = {{MOSuite}: R package for differential multi-omics analysis},
      author = {Kelly Sovacool and Philip Homan and Vishal Koparde and Samantha Chill},
      year = {2025},
      doi = {10.5281/zenodo.16371580},
      url = {https://ccbr.github.io/MOSuite/},
    }
